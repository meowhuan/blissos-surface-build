...
- **libvulkan**：完整 `make` 之后会回退到 API35，必须**构建后补救**：把 `libvulkan.so.api36` 复制到 `out/target/product/x86_64/system/lib64/libvulkan.so` 和 `obj/PACKAGING/target_files_intermediates/bliss_x86_64-target_files/SYSTEM/lib64/libvulkan.so`，再带 `BOARD_IS_SURFACE_BUILD=true make iso_img`。详见 README「构建」一节与 `build.yml`。
